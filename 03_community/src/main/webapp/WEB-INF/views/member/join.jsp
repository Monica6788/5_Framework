...
<%@ taglib prefix="c" uri="jakarta.tags.core" %>
	...
	<%--
		...
		: 전송 데이터(요청 파라미터) 중 파일이 있을 경우 설정하는 옵션
		  이 속성이 없으면 파일의 내용이 전달되지 않음
		  (파일이 있는 경우 form 태그 내에 필수로 지정할 속성!!!)
		  파일의 실제 바이너리 데이터(컴퓨터 전용 날것의 파일 및 데이터)를 함께 보내기 위한 필수 설정
	--%>

	<form id="join-form" class="form form-flex" action="/member/join" method="post" enctype="multipart/form-data">
		...
		<div class="form-row">
			...
			<%-- 비밀번호 확인은 따로 name 속성이 없음 => 서버로 전송 안 할 거임! --%>
