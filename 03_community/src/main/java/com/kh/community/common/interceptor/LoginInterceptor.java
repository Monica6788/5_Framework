 ...
 * 	 이 인터셉터를 통해 로그인이 필요한 경로와 아닌 경로를 분리하여 로그인 여부를 검사함
 */
public class LoginInterceptor implements HandlerInterceptor {
	// 컨트롤러보다 전에 체크해서 넘겨줘야 하니까 preHandle 
	@Override
	public boolean preHandle(HttpServletRequest request, 
			HttpServletResponse response, Object handler) throws Exception {
		// preHandle: 컨트롤러로 진입하는 시점에 동작
		HttpSession session = request.getSession(false);
		// session이 없을 때는 억지로 session을 생성하지 않도록 false 처리
		
		boolean isLoggedIn = session != null 
						&& session.getAttribute(SessionConst.LOGIN_MEMBER) != null;
		
		if (isLoggedIn) {
			return true;	// 로그인 되어 있는 경우 컨트롤러가 그대로 진행하게 함
		}
		// 로그인이 안 되어 있는 경우 로그인 페이지로 리다이렉트 처리 후 컨트롤러 진입 차단
//		response.sendRedirect("/member/login");
		// 기존에 요청했던 주소로 로그인 후 리다이렉트 처리
		String redirectURL = URLEncoder.encode(request.getRequestURI(),
